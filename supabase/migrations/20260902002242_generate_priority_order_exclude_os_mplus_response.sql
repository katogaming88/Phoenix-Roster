-- The recip CTE treated *any* rclc_loot row for a player/item/season as
-- "already received this" -- regardless of the RCLC response that was
-- actually selected. An off-spec (OS) or Mythic+ (M+) roll response isn't a
-- loot council award for that character's main spec, so it shouldn't
-- suppress/soften their future priority for that item the same way an
-- actual MS award does. Loot history/reporting is untouched -- these rows
-- still show up in rclc_loot and the officer report views, they just no
-- longer feed generate_priority_order()'s exclusion/multiplier logic (#856).
--
-- Word-boundary regex, not exact match: response is guild-configurable
-- freeform text (e.g. "Top Pick", "Need" already seen live), so "OS" or
-- "M+" could appear as a standalone token alongside other text rather than
-- being the entire label.
