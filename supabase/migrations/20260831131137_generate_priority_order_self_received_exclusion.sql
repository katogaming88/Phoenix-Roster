-- generate_priority_order()'s exclusion logic only ever looked at
-- public.rclc_loot to decide "does this candidate already have the item at
-- this track" -- that table is populated purely from the addon's raid-loot
-- import. But a raider can also have the item confirmed via the Mark
-- Received flow (public.self_received_requests, approved rows, #406/#386)
-- without it ever landing in rclc_loot: a Great Vault pick, a catalyzed
-- item, a crafted piece, or anything an officer hand-confirms all go through
-- self_received_requests only. Anyone in that state still showed up as a
-- normal candidate on the priority list for that same item/track, since the
-- recip CTE had no way to see it.
--
-- Only the recip CTE changes: it now pools rclc_loot with approved
-- self_received_requests rows for the same item before computing
-- has_myth/has_hero/has_champ, so a self-confirmed receive excludes a
-- candidate exactly the same way an addon-imported award already does.
-- Everything downstream (candidates, base's filter, scored/multiplied,
-- sort order) is unchanged -- it already consumed has_myth/has_hero/has_champ
-- as opaque booleans regardless of source.
--
-- No season filter on the self_received_requests side: unlike rclc_loot, that
-- table carries no season column at all (same gap public.bis_items already
-- has, which the `bis` CTE above has always accepted) -- item identity alone
-- is what's matched on here, consistent with that existing precedent.
