-- import_rclc_loot()'s track parser assumed an instance string always ends
-- "<Name>-<Difficulty>" and took everything after the LAST hyphen as the
-- difficulty word. Sporefall's Mythic difficulty breaks that: RCLC's
-- instance string for it is "Sporefall-Mythic - Flexible Raiding" (a Mythic
-- Flex raid, 15-25 players), so the greedy `regexp_replace(instance, '^.*-',
-- '')` matched through the SECOND hyphen too and left "Flexible Raiding" --
-- not "Mythic" -- as the parsed suffix. track ended up null on every
-- Sporefall row, which the Loot feed then displays as "Other" instead of the
-- real difficulty.
--
-- Fix: search the whole instance string for "mythic"/"heroic"/"normal" as a
-- standalone word (case-insensitive) instead of assuming a fixed
-- "<Name>-<Difficulty>" shape. Robust to whatever comes after the difficulty
-- word (a flex qualifier, or nothing).
--
-- Same 3-arg signature, so this replaces the function in place.
...
begin
  ...
  loop
    ...
    -- "<Name>-<Difficulty>" shape -- see comment at top of file. The RCLC
    -- itemString technically encodes the true track in its bonus IDs, but
    -- decoding those needs a maintained Blizzard bonus-ID reference table
    -- this repo doesn't have -- deferred, not attempted here.
