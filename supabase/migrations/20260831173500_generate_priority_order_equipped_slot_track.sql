-- New signal, additive to the existing recip CTE (same-item loot-award
-- history): whether a candidate already has a Hero-or-better-equivalent
-- item *equipped in this item's slot*, regardless of which specific item it
-- is or where it came from -- e.g. a Hero belt from an earlier boss should
-- mildly deprioritize someone on a different Hero belt drop. Sourced from
-- the new public.player_equipped_gear table (synced from the Blizzard API).
--
-- items.slot only says "Finger"/"Trinket" (a type, not a position), while
-- player_equipped_gear is keyed on the Blizzard API's positional slot
-- vocabulary (FINGER_1/FINGER_2, TRINKET_1/TRINKET_2, MAIN_HAND/OFF_HAND)
-- -- item_slot_map below is the same fan-out BIS_CATALOG_SLOT_TO_ROWS
-- (js/tabs/tab-bis.js) uses, just pointed at those keys instead of
-- BIS_SLOTS rows. Weapon and Off Hand deliberately stay separate, not
-- fanned into each other, same as the existing dual-wield BiS handling
-- (#652's dedupe decision).
--
-- Deliberately compares raw item_level against the season's officer-set
-- Hero/Myth ilvl floor (team_settings.config.trackIlvlThresholds), NOT
-- player_equipped_gear.track -- confirmed with Kat (#845) that a Mythic+/
-- crafted/catalyst piece sitting at Hero-or-Myth-equivalent ilvl in this
-- slot should count the same as an actual raid drop would: the point is
-- "how itemized are they already here," not "did this specific piece come
-- from a raid." `track` on that table stays a display-only label (used only
-- for the has_myth/has_hero same-item status text above, which IS sourced
-- from actual raid awards).
--
-- Multiplier magnitude is deliberately weaker than the same-item
-- has_hero/has_champ multipliers just above it (0.85-1.15): this is "the
-- slot itself is less of an upgrade for them," not "they already own this
-- item" -- a much softer signal, so it stacks on top rather than competing
  ...
  multiplied as (
    select
      ...
      * case when p_track = 'Hero' and has_champ then 0.90 else 1.0 end
      -- Equipped-slot track multipliers (new): a weaker, independent signal
      -- from the same-item ones above -- "already itemized here," not
      -- "already owns this drop."
