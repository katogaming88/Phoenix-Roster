-- 20260826011447_rclc_export_wishlist_status.sql's "wish" CTE filtered
-- item_preferences by "season = p_season", but that compares two different
-- season formats: p_season is the short code (e.g. "MID2", the same format
-- priority_order.season uses), while item_preferences.season is always
-- stamped with the season *display name* (e.g. "Midnight Season 2") --
-- confirmed by the 20260725135340 backfill (team_settings.config->>
-- 'seasonName') and by isItemInSeasonScope() (js/common.js) comparing
-- rowSeason against resolveSeasonView(), never the code. The mismatch meant
-- the join never matched a single row, so every ranked player in the export
-- showed no wishlist status at all (confirmed live against item 351,
-- Jan'thrazet -- reported as a totally blank Full Priority Order panel
-- in-game, tonight, right after re-importing a fresh export).
--
-- Fix: drop the season filter entirely, matching how generate_priority_order()
-- itself already reads item_preferences (20260817135343) -- it doesn't filter
-- by season either. Real items are already season-scoped by their raid zone
-- (not by item_preferences.season, which only exists to scope placeholder
-- rows), so item_id + team_id is the correct match here too.
...
begin
  ...
  wish as (
    select player_id, item_id, status
