-- Suggest Order's officer-facing re-click fix (js/tabs/tab-priority.js
-- prioEditGenerate()'s prioEditFirstPrioSummary logic) only ever nudged the
-- #1 slot away from someone who already held rank 1 elsewhere, and only on
-- a manual re-click. Every OTHER rank in the suggested list, and every FIRST
-- click, still ignored how much priority a candidate already carries across
-- the rest of the priority order entirely -- someone who's already ranked
-- #2-#3 on a dozen other items could still land #1 here over someone
-- holding nothing anywhere, as long as their raw score edged it out. And the
-- reverse problem existed too, unaddressed: nothing ever gave a boost to
-- someone who's consistently ranked low (10+) everywhere -- two candidates
-- with equally little "current stacking" would just fall back to raw score,
-- so a lower scorer could sit at the bottom of every single list forever.
--
-- Adds one hard sort tier, avg_existing_rank, that covers both: for each
-- candidate, the average `rank` across every OTHER item/track they're
-- currently placed on this season (excludes the exact item+track being
-- generated now, so a stale self-reference from the row about to be
-- replaced doesn't count against them). Sorted DESCENDING with nulls first
-- (Postgres's default for desc, kept explicit for clarity) -- a candidate
-- who already averages a great rank (near 1) sorts later here, deprioritized
-- against further stacking; a candidate who averages a poor rank (10+)
-- sorts earlier, boosted; a candidate with NO other placements at all sorts
-- earliest of all (null), since they have the least existing priority of
-- anyone and deserve first shot at getting some. Slotted right after
-- tier-piece catch-up and before raw performance score -- it only breaks
-- ties among candidates who are already equally deserving (same wishlist
-- tag, same tier-piece need), never overrides an actual BiS/tier need with
-- a fairness nudge. Purely an internal sort factor, not a new output
-- column -- return shape is unchanged, so check_priority_order_drift()'s
-- ordinality column list (20260811122020_priority_order_raw_wishlist_status.sql)
-- needs no update.
...
begin
  ...
  existing_priority as (
    ...
    where po.team_id = p_team_id
      ...
      and not (po.item_id = p_item_id and po.track = p_track)
