...
// upload/remove, and that a failure leaves the existing imagePath alone.
...
describe('_bioStoragePathFromImagePath', () => {
  it('extracts the {auth_user_id}/{file} path from a bucket URL', () => {
    const { sandbox } = makeSandbox();
    expect(sandbox._bioStoragePathFromImagePath(BUCKET_PREFIX + 'uid-1/123.jpg')).toBe('uid-1/123.jpg');
  });

  it('returns null for a legacy assets/officers path, not a bucket URL', () => {
    const { sandbox } = makeSandbox();
    expect(sandbox._bioStoragePathFromImagePath('assets/officers/kato.jpg')).toBeNull();
  });

  it('returns null for an empty imagePath', () => {
    const { sandbox } = makeSandbox();
    expect(sandbox._bioStoragePathFromImagePath('')).toBeNull();
  });
});
...
describe('bioRemovePhoto (team editor)', () => {
  it('clears imagePath and calls the delete path for an uploaded bucket photo', async () => {
    const { sandbox } = makeSandbox();
    ...
    await flush();
    expect(calls).toEqual([{ name: 'upload-bio-photo', method: 'DELETE', body: { targetPath: 'uid-1/1.jpg' } }]);
    expect(sandbox.TEAM_OFFICER_BIOS[0].imagePath).toBe('');
  });

  it('clears imagePath without calling invoke for a legacy assets/officers path', async () => {
    const { sandbox } = makeSandbox();
    const { calls, client } = recorderClient();
    sandbox.supabaseClient = client;
    sandbox.TEAM_OFFICER_BIOS = [{ imagePath: 'assets/officers/kato.jpg' }];
...
describe('guildBioUploadPhoto / guildBioRemovePhoto', () => {
  ...
  it('remove clears GUILD_OFFICER_BIOS imagePath and targets the right path', async () => {
    const { sandbox } = makeSandbox();
    const { calls, client } = recorderClient({ error: null });
    sandbox.supabaseClient = client;
    ...
    await flush();
    expect(calls).toEqual([{ name: 'upload-bio-photo', method: 'DELETE', body: { targetPath: 'uid-2/1.jpg' } }]);
