...
- **Donate my finder's cut to the guild** is a checkbox under the note, off by default ([#862](https://github.com/katogaming88/WGA-Raid-Hub/issues/862)). It records the intent: the row shows it to the managers on the BoE Sales page, the Discord post carries it as its own field, and the manager's settle button is what decides when the item sells
