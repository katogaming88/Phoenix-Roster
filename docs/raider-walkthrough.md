...
- **Their BiS plan changed but they can't self-edit their Wishlist right now** -- their BiS link should already be on file as the reference source, so use **My List Changed (Same Link)** to flag it either way: an officer can update the BiS Manager grid directly, or flip their "Allow Wishlist Edit" toggle so they can make the change themselves. Note field is a good place to say which they'd prefer
