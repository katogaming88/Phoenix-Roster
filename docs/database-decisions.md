...
- **The money columns do not change.** `finder_payout` stays what the finder was owed under policy and `guild_cut` the policy remainder, so the recognition is visible and #861's auction house fee applies the same way either route. Reporting counts guild income as `guild_cut` plus `finder_payout` on donated paid rows, and shows the donated total on its own line. A sold row that is donating stays outstanding until a manager settles it: the flag before settlement is intent, and only the button decides.
